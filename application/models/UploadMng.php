...
class UploadMng extends CI_Model {
	...
	{	
		...
		{
			$str_err_msg = strip_tags($this->upload->display_errors());
			return false;
		}
	...
	public function upload_carousel_img()
	{
		...
		$data = $this->upload->data();
		$articleimg_name = 'carousel_'.date('YmdHis', time()).strval(mt_rand(100000, 999999)).$data['file_ext'];
		$articleimg_path = $data['file_path'].'../imgs/carousel/';
		$ret = rename($data['full_path'], $articleimg_path.$articleimg_name);
		if ($ret === false)
		{
			$str_err_msg = 'rename failed';
		...
		return '/imgs/carousel/'.$articleimg_name;
	}
}
